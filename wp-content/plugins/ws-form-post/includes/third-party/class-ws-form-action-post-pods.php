<?php
	...
	class WS_Form_Action_Post_Pods extends WS_Form_Action_Post {
		...
		public function hook_config_meta_keys($config_meta_keys) {
			...
			// Pods - Fields
			$config_meta_keys['action_post_pods_field_id'] = array(

				'label'							=>	__('Pods Field', 'ws-form-post'),
				'type'							=>	'select',
			...
			// Pods - Field mapping
			$config_meta_keys['action_post_field_mapping_pods'] = array(

				'label'						=>	__('Pods Field Mapping', 'ws-form-post'),
				'type'						=>	'repeater',
				'help'						=>	__('Map WS Form fields to Pods fields.', 'ws-form-post'),
				'meta_keys'					=>	array(

					'ws_form_field',
					'action_post_pods_field_id'
				),
			...
			$config_meta_keys['action_post_form_populate_field_mapping_pods'] = array(

				'label'						=>	__('Pods Field Mapping', 'ws-form-post'),
				'type'						=>	'repeater',
				'help'						=>	__('Map Pods field values to WS Form fields.', 'ws-form-post'),
				'meta_keys'					=>	array(
		...
		public function hook_action_settings($settings) {
			...
			array_splice($settings['meta_keys'], 7, 0, 'action_post_field_mapping_pods');
		...
		public function hook_config_settings_form_admin($meta_keys) {
			...
			parent::meta_key_inject($meta_keys, 'action_post_form_populate_field_mapping_pods', 'form_populate_tag_mapping');
		...
		public function hook_list_fields($list_fields, $list_id) {
			...
			// Get fields
			$fields = WS_Form_Pods::pods_get_fields_all('post_type', array($list_id), false, true, false, false);
		...
		public function hook_form_actions($form_actions, $form_field_id_lookup_all, $form_field_type_lookup, $list_id) {
			...
			$form_actions['post']['meta']['action_post_field_mapping_pods'] = array();
			...
			foreach($list_fields as $list_field) {
				...
				$form_actions['post']['meta']['action_post_field_mapping_pods'][] = array(
					...
					'action_post_pods_field_id' => $list_field['id']
				);
			}
		...
		public function hook_form_meta($form_meta, $form_field_id_lookup_all, $list_id) {
			...
			$fields = WS_Form_Pods::pods_get_fields_all('post_type', array($list_id), false, false, true, false);

			$form_meta['action_post_form_populate_field_mapping_pods'] = array();
			...
			foreach($fields as $field) {
				...
				$form_meta['action_post_form_populate_field_mapping_pods'][] = array(
					...
					'action_post_pods_field_id' => $field['value'],
		...
		public function hook_field_mapping($field_mapping_return, $form, $submit, $config, $deduplication_mapping, $list_id) {
			...
			// Field mapping
			$field_mapping_pods = parent::get_config($config, 'action_post_field_mapping_pods', array());
			if(!is_array($field_mapping_pods)) { $field_mapping_pods = array(); }

			// Build pods ID to name lookup
			$pods_id_to_name_lookup = WS_Form_Pods::pods_get_id_to_name_lookup('post_type', $list_id);

			// Run through each field mapping
			foreach($field_mapping_pods as $field_map_pods) {

				// Get Pods field ID
				$pods_field_id = $field_map_pods['action_post_pods_field_id'];
				...
				if(!parent::duplication_repeatable_return($get_submit_value_repeatable_return, $field_id, $pods_field_id, $list_id, $deduplication_mapping)) {
		...
		public function hook_get($return_array, $form, $post_id, $list_id) {
			...
			$field_mapping_pods = WS_Form_Common::get_object_meta_value($form, 'action_post_form_populate_field_mapping_pods', '');
			if(is_array($field_mapping_pods)) {

				// Get Pods field values for current post
				$pods_field_data = WS_Form_Pods::pods_get_field_data('post_type', array($list_id), $post_id);
		...
		public function hook_svg_custom_field_logos($svg_custom_field_logos, $list_id) {
			...
			if(WS_Form_Pods::pods_get_fields_all('post_type', array($list_id), false, true, false, true)) {
