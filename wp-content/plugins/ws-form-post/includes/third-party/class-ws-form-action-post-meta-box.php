<?php
	...
	class WS_Form_Action_Post_Meta_Box extends WS_Form_Action_Post {
		...
		public function hook_config_meta_keys($config_meta_keys) {
			...
			// Meta Box - Fields
			$config_meta_keys['action_post_meta_box_field_id'] = array(

				'label'							=>	__('Meta Box Field', 'ws-form-post'),
				'type'							=>	'select',
			...
			$config_meta_keys['action_post_field_mapping_meta_box'] = array(

				'label'						=>	__('Meta Box Field Mapping', 'ws-form-post'),
				'type'						=>	'repeater',
				'help'						=>	__('Map WS Form fields to Meta Box fields.', 'ws-form-post'),
				'meta_keys'					=>	array(
			...
			$config_meta_keys['action_post_form_populate_field_mapping_meta_box'] = array(

				'label'						=>	__('Meta Box Field Mapping', 'ws-form-post'),
				'type'						=>	'repeater',
				'help'						=>	__('Map Meta Box field values to WS Form fields.', 'ws-form-post'),
				'meta_keys'					=>	array(
		...
		public function hook_action_settings($settings) {
			...
			array_splice($settings['meta_keys'], 7, 0, 'action_post_field_mapping_meta_box');
		...
		public function hook_config_settings_form_admin($meta_keys) {
			...
			parent::meta_key_inject($meta_keys, 'action_post_form_populate_field_mapping_meta_box', 'form_populate_tag_mapping');
		...
		public function hook_list_fields($list_fields, $list_id) {
			...
			// Get fields
			$fields = WS_Form_Meta_Box::meta_box_get_fields_all('post', array($list_id), false, true, false, false);
		...
		public function hook_form_actions($form_actions, $form_field_id_lookup_all, $form_field_type_lookup, $list_id) {
			...
			$form_actions['post']['meta']['action_post_field_mapping_meta_box'] = array();
			...
			foreach($list_fields as $list_field) {
				...
				$form_actions['post']['meta']['action_post_field_mapping_meta_box'][] = array(
					...
					'action_post_meta_box_field_id' => $list_field['id']
				);
			}
		...
		public function hook_form_meta($form_meta, $form_field_id_lookup_all, $list_id) {
			...
			$fields = WS_Form_Meta_Box::meta_box_get_fields_all('post', array($list_id), false, false, true, false);

			$form_meta['action_post_form_populate_field_mapping_meta_box'] = array();
			...
			foreach($fields as $field) {
				...
				$form_meta['action_post_form_populate_field_mapping_meta_box'][] = array(
					...
					'action_post_meta_box_field_id' => $field['value'],
		...
		public function hook_field_mapping($field_mapping_return, $form, $submit, $config, $deduplication_mapping, $list_id) {
			...
			// Field mapping
			$field_mapping_meta_box = parent::get_config($config, 'action_post_field_mapping_meta_box', array());
			...
			foreach($field_mapping_meta_box as $field_map_meta_box) {
				...
				// Get Meta Box key
				$meta_box_field_id = $field_map_meta_box['action_post_meta_box_field_id'];
				...
				if(!parent::duplication_repeatable_return($get_submit_value_repeatable_return, $field_id, $meta_box_field_id, $list_id, $deduplication_mapping)) {
		...
		public function hook_file($file, $attachment_id) {
			...
			if(isset($this->meta_box_file_fields[$field_id])) {
				...
				// Get Meta Box attachments key (Used to group attachment ID's together)
		...
		public function hook_post_meta($form, $submit, $config, $post_id, $list_id, $taxonomy_tags) {
			...
			$meta_box_data_array = array();

			// Add slashes
			$this->meta_box_update_fields = wp_slash($this->meta_box_update_fields);

			// Update fields
			foreach($this->meta_box_update_fields as $meta_box_field_id => $meta_box_meta_value) {
				...
				// Store according storage type
				switch($meta_box_storage_type) {

					case 'custom_table' :

						if(!isset($meta_box_data_array[$meta_box_table])) {

							$meta_box_data_array[$meta_box_table] = array();
						}

						if(is_array($meta_box_meta_value)) {

							$meta_box_meta_value = serialize($meta_box_meta_value);
						}

						$meta_box_data_array[$meta_box_table][$meta_box_field_id] = $meta_box_meta_value;

						break;

					default :

						rwmb_set_meta($post_id, $meta_box_field_id, $meta_box_meta_value);
				}
			}

			// Custom table processing
			if(count($meta_box_data_array) > 0) {

				foreach($meta_box_data_array as $meta_box_table => $meta_box_data) {

					try {

						$exists = \MetaBox\CustomTable\API::exists($post_id, $meta_box_table);

						if($exists) {

							$api_return = \MetaBox\CustomTable\API::update($post_id, $meta_box_table, $meta_box_data);

						} else {

							$api_return = \MetaBox\CustomTable\API::add($post_id, $meta_box_table, $meta_box_data);
						}
						...
						if($api_return === false) {
							...
							// Error
							global $wpdb;
							parent::error_js(sprintf(

								$exists ? __('Meta Box custom table update record failed: %s', 'ws-form-post') : __('Meta Box custom table add record failed: %s', 'ws-form-post'),
		...
		public function hook_get($return_array, $form, $post_id, $list_id) {
			...
			$field_mapping_meta_box = WS_Form_Common::get_object_meta_value($form, 'action_post_form_populate_field_mapping_meta_box', '');
			if(is_array($field_mapping_meta_box)) {

				// Get Meta Box field values for current post
				$meta_box_field_data = WS_Form_Meta_Box::meta_box_get_field_data('post', array($list_id), $post_id);
		...
		public function hook_svg_custom_field_logos($svg_custom_field_logos, $list_id) {
			...
			if(WS_Form_Meta_Box::meta_box_get_fields_all('post', array($list_id), false, true, false, true)) {
	...
	new WS_Form_Action_Post_Meta_Box();
