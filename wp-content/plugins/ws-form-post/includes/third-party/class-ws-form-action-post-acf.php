<?php
	...
	class WS_Form_Action_Post_ACF extends WS_Form_Action_Post {
		...
		public function hook_config_meta_keys($config_meta_keys) {
			...
			// ACF - Fields
			$config_meta_keys['action_post_acf_key'] = array(
			...
			// ACF - Field mapping
			$config_meta_keys['action_post_field_mapping_acf'] = array(
			...
			// ACF - Validation 
			$config_meta_keys['action_post_acf_validation'] = array(

				'label'						=>	__('Process ACF Validation', 'ws-form'),
				'type'						=>	'checkbox',
				'help'						=>	__('Enabling this will process ACF validation filters when the form is submitted.', 'ws-form-post'),
				'default'					=>	'',
				'condition'					=>	array(
			...
			$config_meta_keys['action_post_form_populate_field_mapping_acf'] = array(
		...
		public function hook_action_settings($settings) {
			...
			array_splice($settings['meta_keys'], 7, 0, 'action_post_acf_validation');
			array_splice($settings['meta_keys'], 7, 0, 'action_post_field_mapping_acf');
		...
		public function hook_config_settings_form_admin($meta_keys) {
			...
			parent::meta_key_inject($meta_keys, 'action_post_form_populate_field_mapping_acf', 'form_populate_tag_mapping');
		...
		public function hook_list_fields($list_fields, $list_id) {
			...
			// Get fields
			$fields = WS_Form_ACF::acf_get_fields_all(array('post_type' => $list_id), false, true, false, false);
		...
		public function hook_form_actions($form_actions, $form_field_id_lookup_all, $form_field_type_lookup, $list_id) {
			...
			$form_actions['post']['meta']['action_post_field_mapping_acf'] = array();
			...
			foreach($list_fields as $list_field) {
				...
				$form_actions['post']['meta']['action_post_field_mapping_acf'][] = array(
					...
					'action_post_acf_key' => $list_field['id']
				);
			}
		...
		public function hook_form_meta($form_meta, $form_field_id_lookup_all, $list_id) {
			...
			$fields = WS_Form_ACF::acf_get_fields_all(array('post_type' => $list_id), false, false, true, false);

			$form_meta['action_post_form_populate_field_mapping_acf'] = array();
			...
			foreach($fields as $field) {
				...
				$form_meta['action_post_form_populate_field_mapping_acf'][] = array(
					...
					'action_post_acf_key' => $field['value'],
		...
		public function hook_field_mapping($field_mapping_return, $form, $submit, $config, $deduplication_mapping, $list_id) {
			...
			// Field mapping
			$field_mapping_acf = parent::get_config($config, 'action_post_field_mapping_acf', array());
			if(!is_array($field_mapping_acf)) { $field_mapping_acf = array(); }

			// ACF validation
			$acf_validation = parent::get_config($config, 'action_post_acf_validation', '');
			...
			foreach($field_mapping_acf as $field_map_acf) {
				...
				// Get ACF key
				$acf_key = $field_map_acf['action_post_acf_key'];
				...
				if(
					...
					isset($field_object['name'])
				) {
					// Run through each value and check for duplication
					if(!parent::duplication_repeatable_return($get_submit_value_repeatable_return, $field_id, $field_object['name'], $list_id, $deduplication_mapping)) {
		...
		public function hook_get($return_array, $form, $post_id, $list_id) {
			...
			$field_mapping_acf = WS_Form_Common::get_object_meta_value($form, 'action_post_form_populate_field_mapping_acf', '');
		...
		public function hook_svg_custom_field_logos($svg_custom_field_logos, $list_id) {
			...
			if(WS_Form_ACF::acf_get_fields_all(array('post_type' => $list_id), false, true, false, true)) {
