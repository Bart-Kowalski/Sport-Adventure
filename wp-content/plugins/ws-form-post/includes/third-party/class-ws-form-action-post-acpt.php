<?php
	...
	class WS_Form_Action_Post_ACPT extends WS_Form_Action_Post {
		...
		public function hook_config_meta_keys($config_meta_keys) {
			...
			// ACPT - Fields
			$config_meta_keys['action_post_acpt_field_id'] = array(

				'label'							=>	__('ACPT Field', 'ws-form-post'),
				'type'							=>	'select',
				'options'						=>	is_admin() ? WS_Form_ACPT::acpt_get_fields_all('posts', false, false, true, false) : array(),
				'options_blank'					=>	__('Select...', 'ws-form-post')
			);

			// ACPT - Field mapping
			$config_meta_keys['action_post_field_mapping_acpt'] = array(

				'label'						=>	__('ACPT Field Mapping', 'ws-form-post'),
				'type'						=>	'repeater',
				'help'						=>	__('Map WS Form fields to ACPT fields.', 'ws-form-post'),
				'meta_keys'					=>	array(

					'ws_form_field',
					'action_post_acpt_field_id'
				),
			...
			$config_meta_keys['action_post_form_populate_field_mapping_acpt'] = array(

				'label'						=>	__('ACPT Field Mapping', 'ws-form-post'),
				'type'						=>	'repeater',
				'help'						=>	__('Map ACPT field values to WS Form fields.', 'ws-form-post'),
				'meta_keys'					=>	array(
		...
		public function hook_action_settings($settings) {
			...
			array_splice($settings['meta_keys'], 7, 0, 'action_post_field_mapping_acpt');
		...
		public function hook_config_settings_form_admin($meta_keys) {
			...
			parent::meta_key_inject($meta_keys, 'action_post_form_populate_field_mapping_acpt', 'form_populate_tag_mapping');
		...
		public function hook_list_fields($list_fields, $list_id) {
			...
			// Get fields
			$fields = WS_Form_ACPT::acpt_get_fields_all($list_id, false, true, false, false);
		...
		public function hook_form_actions($form_actions, $form_field_id_lookup_all, $form_field_type_lookup, $list_id) {
			...
			$form_actions['post']['meta']['action_post_field_mapping_acpt'] = array();
			...
			foreach($list_fields as $list_field) {
				...
				$form_actions['post']['meta']['action_post_field_mapping_acpt'][] = array(
					...
					'action_post_acpt_field_id' => $list_field['id']
				);
			}
		...
		public function hook_form_meta($form_meta, $form_field_id_lookup_all, $list_id) {
			...
			$fields = WS_Form_ACPT::acpt_get_fields_all($list_id, false, false, true, false);

			$form_meta['action_post_form_populate_field_mapping_acpt'] = array();
			...
			foreach($fields as $field) {
				...
				$form_meta['action_post_form_populate_field_mapping_acpt'][] = array(
					...
					'action_post_acpt_field_id' => $field['value'],
		...
		public function hook_field_mapping($field_mapping_return, $form, $submit, $config, $deduplication_mapping, $list_id) {
			...
			// Field mapping
			$field_mapping_acpt = parent::get_config($config, 'action_post_field_mapping_acpt', array());
			...
			foreach($field_mapping_acpt as $field_map_acpt) {
				...
				if(
					!isset($field_map_acpt['action_post_acpt_field_id']) ||
				...
				// Get ACPT name
				$acpt_field_id = $field_map_acpt['action_post_acpt_field_id'];
				...
				// Get ACPT field type
				$acpt_field_type = WS_Form_ACPT::acpt_get_field_type($acpt_field_id, $list_id);
				if($acpt_field_type === false) { continue; }

				// Get ACPT field name
				$acpt_field_name = WS_Form_ACPT::acpt_get_field_name($acpt_field_id, $list_id);
				if($acpt_field_name === false) { continue; }

				// Get parent ACF field type
				$acpt_data = WS_Form_ACPT::acpt_get_parent_data($acpt_field_id, $list_id);
				...
				$acpt_parent_data = WS_Form_ACPT::acpt_get_parent_data($acpt_field_id, $list_id);
				...
				switch($acpt_parent_field_type) {
					...
					case 'Repeater' :
						...
						foreach($get_submit_value_repeatable_return['value'] as $repeater_index => $meta_value) {
							...
							// Get field name
							$field_name = WS_Form_ACPT::acpt_get_field_name($acpt_field_id, $list_id);
		...
		public function hook_file($file, $attachment_id) {
			...
			if(isset($this->acpt_file_fields[$field_id])) {
				...
				$acpt_field_type = WS_Form_ACPT::acpt_get_field_type($acpt_field_id, 'posts');
		...
		public function hook_post_meta($form, $submit, $config, $post_id, $list_id, $taxonomy_tags) {
			...
			foreach($this->acpt_update_fields as $acpt_field_id => $acpt_field_value) {
				...
				// Get box name
				$box_name = WS_Form_ACPT::acpt_get_box_name($acpt_field_id, $list_id);
		...
		public function hook_get($return_array, $form, $post_id, $list_id) {
			...
			$field_mapping_acpt = WS_Form_Common::get_object_meta_value($form, 'action_post_form_populate_field_mapping_acpt', '');
			if(is_array($field_mapping_acpt)) {

				// Get ACPT field values for current post
				$acpt_field_data = WS_Form_ACPT::acpt_get_field_data($list_id, $post_id);
		...
		public function hook_svg_custom_field_logos($svg_custom_field_logos, $list_id) {
			...
			if(WS_Form_ACPT::acpt_get_fields_all($list_id, false, true, false, true)) {
