<?php
	...
	class WS_Form_Action_Post_JetEngine extends WS_Form_Action_Post {
		...
		public function hook_config_meta_keys($config_meta_keys) {
			...
			$config_meta_keys['action_post_field_mapping_jetengine'] = array(

				'label'						=>	__('JetEngine Field Mapping', 'ws-form-post'),
				'type'						=>	'repeater',
				'help'						=>	__('Map WS Form fields to JetEngine fields.', 'ws-form-post'),
				'meta_keys'					=>	array(

					'ws_form_field',
					'action_post_jetengine_field_name'
				),
			...
			$config_meta_keys['action_post_form_populate_field_mapping_jetengine'] = array(

				'label'						=>	__('JetEngine Field Mapping', 'ws-form-post'),
				'type'						=>	'repeater',
				'help'						=>	__('Map JetEngine field values to WS Form fields.', 'ws-form-post'),
				'meta_keys'					=>	array(
			...
			// JetEngine - Relations
			$config_meta_keys['action_post_jetengine_relations'] = array(

				'label'						=>	__('JetEngine Relations', 'ws-form-post'),
				'type'						=>	'repeater',
				'help'						=>	__('Set a JetEngine relation parent or child value to a field value.', 'ws-form-post'),
				'meta_keys'					=>	array(

					'ws_form_field',
					'action_post_jetengine_relation_id',
					'action_post_jetengine_relation_context',
			...
			// JetEngine - Relations
			$config_meta_keys['action_post_jetengine_relations_populate'] = array(

				'label'						=>	__('JetEngine Relations', 'ws-form-post'),
				'type'						=>	'repeater',
				'help'						=>	__('Set a field to the value of a JetEngine relation parent or child value.', 'ws-form-post'),
				'meta_keys'					=>	array(
			...
			$config_meta_keys['action_post_jetengine_relation_id'] = array(

				'label'							=>	__('Relation', 'ws-form-post'),
				'type'							=>	'select',
				'options'						=>	array()
			);

			// JetEngine - Relations - Context
			$config_meta_keys['action_post_jetengine_relation_context'] = array(

				'label'							=>	__('Context', 'ws-form-post'),
				'type'							=>	'select',
				'options'						=>	array(

					array('value' => 'parent', 'text' => __('Parent', 'ws-form-post')),
					array('value' => 'child', 'text' => __('Child', 'ws-form-post'))
				),
				'default'						=>	'child'
			);

			// JetEngine - Relations - Replace
			$config_meta_keys['action_post_jetengine_relation_replace'] = array(

				'label'						=>	__('Replace', 'ws-form-post'),
				'type'						=>	'select',
				'options'					=>	array(

					array('value' => '', 'text' => __('No', 'ws-form-post')),
					array('value' => 'on', 'text' => __('Yes', 'ws-form-post'))
				),
				'default'					=>	'on'
			);

			// JetEngine - Fields
			$config_meta_keys['action_post_jetengine_field_name'] = array(

				'label'							=>	__('JetEngine Field', 'ws-form-post'),
				'type'							=>	'select',
				'options'						=>	is_admin() ? WS_Form_JetEngine::jetengine_get_fields_all('post_type', null, false, false, true, false) : array(),
			...
			if(is_admin()) {
				...
				if(is_array($relations)) {
					...
					foreach($relations as $rel_id => $rel) {
						...
						if(
							jet_engine()->relations->types_helper->object_is($rel->get_args('parent_object'), 'posts') ||
							jet_engine()->relations->types_helper->object_is($rel->get_args('child_object'), 'posts')
						) {
							$config_meta_keys['action_post_jetengine_relation_id']['options'][] = array(
		...
		public function hook_action_settings($settings) {
			...
			array_splice($settings['meta_keys'], 7, 0, 'action_post_jetengine_relations');
			array_splice($settings['meta_keys'], 7, 0, 'action_post_field_mapping_jetengine');
		...
		public function hook_config_settings_form_admin($meta_keys) {
			...
			parent::meta_key_inject($meta_keys, 'action_post_form_populate_field_mapping_jetengine', 'form_populate_tag_mapping');
			parent::meta_key_inject($meta_keys, 'action_post_jetengine_relations_populate', 'form_populate_tag_mapping');
		...
		public function hook_list_fields($list_fields, $list_id) {
			...
			// Get fields
			$fields = WS_Form_JetEngine::jetengine_get_fields_all('post_type', $list_id, false, true, false, false);
		...
		public function hook_form_actions($form_actions, $form_field_id_lookup_all, $form_field_type_lookup, $list_id) {
			...
			$form_actions['post']['meta']['action_post_field_mapping_jetengine'] = array();
			...
			foreach($list_fields as $list_field) {
				...
				$form_actions['post']['meta']['action_post_field_mapping_jetengine'][] = array(
					...
					'action_post_jetengine_field_name' => $list_field['id']
				);
			}
		...
		public function hook_form_meta($form_meta, $form_field_id_lookup_all, $list_id) {
			...
			$fields = WS_Form_JetEngine::jetengine_get_fields_all('post_type', $list_id, false, false, true, false);

			$form_meta['action_post_form_populate_field_mapping_jetengine'] = array();
			...
			foreach($fields as $field) {
				...
				$form_meta['action_post_form_populate_field_mapping_jetengine'][] = array(
					...
					'action_post_jetengine_field_name' => $field['value'],
		...
		public function hook_field_mapping($field_mapping_return, $form, $submit, $config, $deduplication_mapping, $list_id) {
			...
			// Field mapping
			$field_mapping_jetengine = parent::get_config($config, 'action_post_field_mapping_jetengine', array());
			...
			foreach($field_mapping_jetengine as $field_map_jetengine) {
				...
				$jetengine_field_name = $field_map_jetengine['action_post_jetengine_field_name'];
				...
				if(!parent::duplication_repeatable_return($get_submit_value_repeatable_return, $field_id, $jetengine_field_name, $list_id, $deduplication_mapping)) {

					return 'halt';
				}

				// Get JetEngine field type
				$jetengine_field_type = WS_Form_JetEngine::jetengine_get_field_type($jetengine_field_name);
				if($jetengine_field_type === false) { continue; }

				// Get parent JetEngine field type
				$jetengine_data = WS_Form_JetEngine::jetengine_get_parent_data($jetengine_field_name);
				...
				switch($jetengine_parent_field_type) {
					...
					case 'repeater' :
						...
						foreach($get_submit_value_repeatable_return['value'] as $repeater_index => $meta_value) {
							...
							// Process meta value
							$meta_value = WS_Form_JetEngine::jetengine_ws_form_field_value_to_jetengine_meta_value($meta_value, $jetengine_field_type, $jetengine_field_name, $field_id, $fields, $field_types);
		...
		public function hook_file($file, $attachment_id) {
			...
			if(isset($this->jetengine_file_fields[$field_id])) {
				...
				$jetengine_field_settings = WS_Form_JetEngine::jetengine_get_field_settings($jetengine_field_name);
		...
		public function hook_post_meta($form, $submit, $config, $post_id, $list_id, $taxonomy_tags) {
			...
			// JetEngine relations
			$jetengine_relations = parent::get_config($config, 'action_post_jetengine_relations', array());
			...
			foreach($jetengine_relations as $jetengine_relation) {
				...
				$jetengine_relation_id = $jetengine_relation['action_post_jetengine_relation_id'];
				...
				$jetengine_relation_context = $jetengine_relation['action_post_jetengine_relation_context'];
				...
				$jetengine_relation_replace = ($jetengine_relation['action_post_jetengine_relation_replace'] == 'on');

				// Convert to array
				if(!is_array($submit_value)) { $submit_value = explode(',', $submit_value); }
		...
		public function hook_get($return_array, $form, $post_id, $list_id) {
			...
			$field_mapping_jetengine = WS_Form_Common::get_object_meta_value($form, 'action_post_form_populate_field_mapping_jetengine', '');
			if(is_array($field_mapping_jetengine)) {

				// Get JetEngine field values for current post
				$jetengine_field_data = WS_Form_JetEngine::jetengine_get_field_data('post_type', $list_id, $post_id);
				...
				foreach($field_mapping_jetengine as $field_map_jetengine) {
					...
					$jetengine_field_values = WS_Form_JetEngine::jetengine_jetengine_meta_value_to_ws_form_field_value($jetengine_field_values, $jetengine_field_type, $jetengine_field_repeater, $jetengine_field_name, $field_id, $fields, $field_types);
			...
			$jetengine_relations_populate = WS_Form_Common::get_object_meta_value($form, 'action_post_jetengine_relations_populate', '');
				...
				foreach($jetengine_relations_populate as $jetengine_relation) {
					...
					switch($jetengine_relation_context) {
						...
						case 'parent' :
							...
							$jetengine_field_values = $relation_instance->get_parents($post_id, 'ids');
							break;

						case 'child' :

							$jetengine_field_values = $relation_instance->get_children($post_id, 'ids');
							break;
					}
		...
		public function hook_svg_custom_field_logos($svg_custom_field_logos, $list_id) {
			...
			if(WS_Form_JetEngine::jetengine_get_fields_all('post_type', $list_id, false, true, false, true)) {
	...
	new WS_Form_Action_Post_JetEngine();
