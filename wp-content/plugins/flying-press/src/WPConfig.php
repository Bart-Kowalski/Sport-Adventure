...
class WPConfig
{
  ...
  {
    // Return early if the constant already exists with the same value
    ...
    $regex = '/^\s*define\(\s*[\'"]' . $escaped_name . '[\'"].*$/m';
    ...
    // Insert new define
    $define = "\ndefine( '$name', $formatted_value ); // Added by FlyingPress";
    $content = str_replace('<?php', '<?php' . $define, $content);
    file_put_contents($wp_config_path, $content, LOCK_EX);
  }
  ...
  public static function remove_constant($constant_name)
  {
    ...
    // Remove the define line for the given constant (only if added by FlyingPress)
    $regex =
      '/^\s*define\(\s*[\'"]' . $escaped_constant . '[\'"].*?\/\/\s*Added by FlyingPress\s*$/m';
    $content = preg_replace($regex, '', $content);
