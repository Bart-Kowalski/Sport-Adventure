...
function sa_handle_direct_checkout_redirect() {
    ...
    // Only remove add to cart handler, keep update cart handler for removals
    remove_action('wp_loaded', array('WC_Form_Handler', 'add_to_cart_action'), 20);
    
    // Add our own handler with lower priority
    add_action('wp_loaded', 'sa_handle_add_to_cart_action', 19);
...
// Our custom add to cart handler
function sa_handle_add_to_cart_action() {
    if (empty($_REQUEST['add-to-cart']) || !is_numeric($_REQUEST['add-to-cart'])) {
        return;
    }
    
    $product_id = apply_filters('woocommerce_add_to_cart_product_id', absint($_REQUEST['add-to-cart']));
    $was_added = WC()->cart->add_to_cart($product_id);
...
function sport_adventure_custom_settings_link($links) {
    $settings_link = '<a href="admin.php?page=sport-adventure-debug-settings">Debug Settings</a>';
    array_unshift($links, $settings_link);
    return $links;
}
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'sport_adventure_custom_settings_link');

// Add debug settings page
function sport_adventure_custom_admin_menu() {
    add_options_page(
        'Sport Adventure Debug Settings',
        'SA Debug Settings',
        'manage_options',
        'sport-adventure-debug-settings',
        'sport_adventure_custom_debug_settings_page'
    );
}
add_action('admin_menu', 'sport_adventure_custom_admin_menu');

// Register debug settings
function sport_adventure_custom_register_settings() {
    register_setting('sport_adventure_debug_options', 'sa_debug_enabled');
    register_setting('sport_adventure_debug_options', 'sa_js_debug_enabled');
    register_setting('sport_adventure_debug_options', 'sa_php_debug_enabled');
}
add_action('admin_init', 'sport_adventure_custom_register_settings');

// Debug settings page
function sport_adventure_custom_debug_settings_page() {
    ?>
    <div class="wrap">
        <h1>Sport Adventure Debug Settings</h1>
        <form method="post" action="options.php">
            <?php
            settings_fields('sport_adventure_debug_options');
            do_settings_sections('sport_adventure_debug_options');
            ?>
            <table class="form-table">
                <tr>
                    <th scope="row">Enable PHP Debug Logging</th>
                    <td>
                        <input type="checkbox" name="sa_php_debug_enabled" value="1" <?php checked(1, get_option('sa_php_debug_enabled', 0)); ?> />
                        <p class="description">Enable PHP error_log() statements throughout the plugin</p>
                    </td>
                </tr>
                <tr>
                    <th scope="row">Enable JavaScript Debug Logging</th>
                    <td>
                        <input type="checkbox" name="sa_js_debug_enabled" value="1" <?php checked(1, get_option('sa_js_debug_enabled', 0)); ?> />
                        <p class="description">Enable console.log() statements in JavaScript files</p>
                    </td>
                </tr>
                <tr>
                    <th scope="row">Enable General Debug Mode</th>
                    <td>
                        <input type="checkbox" name="sa_debug_enabled" value="1" <?php checked(1, get_option('sa_debug_enabled', 0)); ?> />
                        <p class="description">Enable general debug mode for the plugin</p>
                    </td>
                </tr>
            </table>
            <?php submit_button(); ?>
        ...
        <div class="card" style="margin-top: 20px;">
            <h2>Current Debug Status</h2>
            <p><strong>PHP Debug:</strong> <?php echo get_option('sa_php_debug_enabled', 0) ? 'Enabled' : 'Disabled'; ?></p>
            <p><strong>JavaScript Debug:</strong> <?php echo get_option('sa_js_debug_enabled', 0) ? 'Enabled' : 'Disabled'; ?></p>
            <p><strong>General Debug:</strong> <?php echo get_option('sa_debug_enabled', 0) ? 'Enabled' : 'Disabled'; ?></p>
        </div>
    </div>
    <?php
}
