...
function add_payment_info_section() {
    ...
    if (!empty($variations[0]['acf_fields'])) {
        ...
        jQuery(document).ready(function($) {
            ...
            // Select first option if no default is set
            $form.on('wc_variation_form', function() {
                var $select = $(this).find('select[name^="attribute_"]');
                if ($select.val() === '') {
                    $select.find('option:eq(1)').prop('selected', true).trigger('change');
...
// Sort variant selector options by date
add_action('wp_footer', function() {
    ...
    jQuery(document).ready(function($) {
        ...
        function sortTerminOptions($select) {
        ...
        // Initial sort
        sortTerminOptions($('#pa_termin'));

        // Sort on any dynamic updates
        $(document).on('wc_variation_form', function() {
            sortTerminOptions($('#pa_termin'));
        });

        // Sort after AJAX requests
