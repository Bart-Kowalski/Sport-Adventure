			...
			foreach($this->meta_box_update_fields as $meta_box_field_id => $meta_box_meta_value) {
				...
				switch($meta_box_storage_type) {
					...
					default :
						...
						rwmb_set_meta($user_id, $meta_box_field_id, $meta_box_meta_value, ['object_type' => 'user']);
				}
			}
			...
			if(count($meta_box_data_array) > 0) {
				...
				foreach($meta_box_data_array as $meta_box_table => $meta_box_data) {
					...
					try {
						...
						$exists = \MetaBox\CustomTable\API::exists($user_id, $meta_box_table);

						if($exists) {

							$api_return = \MetaBox\CustomTable\API::update($user_id, $meta_box_table, $meta_box_data);

						} else {

							$api_return = \MetaBox\CustomTable\API::add($user_id, $meta_box_table, $meta_box_data);
						}
						...
						if($api_return === false) {
							...
							parent::error_js(sprintf(
								...
								$exists ? __('Meta Box custom table update record failed: %s', 'ws-form-user') : __('Meta Box custom table add record failed: %s', 'ws-form-user'),
